    ...
    try:
        ...
        if CP.PhaseSI('S', S4, 'T', T4, fluid) == 'gas':
            ...
            if fluid in {"R404A", "R407C"}:
                return jsonify({
                    'error': 'Two-phase values for pseudo-pure substances'
                             ' (R404A, R407C) are not supported!'
                })
            T1g = T4
            H1g = H4
        ...
        return jsonify({
            ...
            'states': [
                {
                    'state': 1, 'T': round(T1, 2), 'P': round(P1, 2),
                    'V': f'{V1:.8f}', 'H': round(H1, 1), 'S': round(S1, 4),
                    'phase': phase1, 'X': f'{X1:.4f}'
                },
                {
                    'state': 2, 'T': round(T2, 2), 'P': round(P2, 2),
                    'V': f'{V2:.8f}', 'H': round(H2, 1), 'S': round(S2, 4),
                    'phase': phase2, 'X': f'{X2:.4f}'
                },
                {
                    'state': 3, 'T': round(T3, 2), 'P': round(P3, 2),
                    'V': f'{V3:.8f}', 'H': round(H3, 1), 'S': round(S3, 4),
                    'phase': phase3, 'X': f'{X3:.4f}'
                },
                {
                    'state': 4, 'T': round(T4, 2), 'P': round(P4, 2),
                    'V': f'{V4:.8f}', 'H': round(H4, 1), 'S': round(S4, 4),
                    'phase': phase4, 'X': f'{X4_disp:.4f}'
                },
            ],
            'performance': {
                'morc': round(morc, 4),
        ...
        return jsonify({'error': f'Cycle cannot be completed! {e}'})
